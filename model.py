...
class CMAE(nn.Module):
    def __init__(self, in_dim, out_dim, rate, hidden, t, layers, alpha):
    ...
    def forward(self, graph, feat):
        ...
        re_h = h.clone()
        re_h[mask_nodes] = 0.0
        re_x, _ = self.decoder(graph, re_h)
        # re_x = self.decoder(h)

        # Contrastive
        _, ch = self.contrast(graph, feat)
        c_h = self.proj_head(ch)
        c_m = self.pred_head(gh)
        cl_loss = self.cl_loss(c_h, c_m, self.t)

        loss = self.criterion(re_x[mask_nodes], feat[mask_nodes].detach()) \
               + cl_loss * self.alpha
        # loss = cl_loss * self.alpha
        # embeds1 = c_h.clone()
        # embeds2 = c_m.clone()
