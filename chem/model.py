...
class CG(torch.nn.Module):
    def __init__(self, layers, out_dim, drop_ratio=0, gnn_type="gin", alpha=0.5):
        super(CG, self).__init__()
        self.gnn = GNN(layers, out_dim, drop_ratio=drop_ratio)
        self.cl = GNN(layers, out_dim, drop_ratio=drop_ratio)
        self.decoder = GNNDecoder(out_dim, 119)
        self.proj_head = nn.Sequential(nn.Linear(300, 256), nn.ReLU(inplace=True),
                                       nn.Linear(256, 128))
        self.pred_head = nn.Sequential(nn.Linear(300, 256), nn.ReLU(inplace=True),
                                       nn.Linear(256, 128))
        self.alpha = alpha
        # stop gradient
    ...
    def trainable_parameters(self):
        ...
        return list(self.gnn.parameters()) + list(self.proj_head.parameters())\
    ...
    def update_target_network(self, mm):
        ...
        """
        for param_q, param_k in zip(self.gnn.parameters(), self.cl.parameters()):
    ...
    def forward(self, data):
        h = self.gnn(data)
        node_attr_label = data.node_attr_label
        masked_node_indices = data.masked_atom_indices
        pred_node = self.decoder(h, data.edge_index, data.edge_attr, masked_node_indices)
        loss = sce_loss(node_attr_label, pred_node[masked_node_indices])

        # cl
        z1 = global_mean_pool(h, data.batch)
        z1 = self.proj_head(z1)

        z2 = self.cl(data)
        z2 = global_mean_pool(z2, data.batch)
        z2 = self.pred_head(z2)
        cl_loss = self.cl_loss(z1, z2)

        return loss + self.alpha*cl_loss
...
class GNN_graphpred(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, num_tasks, JK="last", drop_ratio=0, graph_pooling="mean", gnn_type="gin"):
        ...
        self.gnn = CG(num_layer, emb_dim, drop_ratio=drop_ratio)
    ...
    def forward(self, *argv):
        ...
        node_representation = self.gnn.gnn(x, edge_index, edge_attr)
